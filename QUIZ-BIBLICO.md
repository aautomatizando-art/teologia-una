...
com banco de dados **Supabase** e integração com **ESP32** (até 8 jogadores).

- **Online:** `https://<seu-projeto>.vercel.app/quiz`
- **Local:** abra `quiz-biblico.html` direto no navegador (funciona offline, com perguntas embutidas e teclado).
...
5. **Resultado:**
   ...
   - ❌ **Errou / tempo esgotado** → mostra a resposta certa e sorteia um **castigo bíblico**
     (penalidade edificante), tocando o som de erro.
6. **Placar** lateral com ranking, acertos e aproveitamento.

### Níveis
- 🟢 **Básico** (10 pts) · 🟡 **Médio** (20 pts) · 🔴 **Avançado** (30 pts)
...
### Protocolo
- ESP → Dashboard: `{"event":"button","btn":2}` ao apertar; estado a cada 200 ms.
- Dashboard → ESP: `{"cmd":"sorteio","player":3}`, `{"cmd":"alloff"}`, `{"cmd":"resultado","ok":true}`.
...
2. Em **SQL Editor**, rode o conteúdo de [`quiz-biblico-setup.sql`](quiz-biblico-setup.sql)
   (cria as tabelas, políticas RLS, ~36 perguntas e os castigos).
...
> Para adicionar perguntas, insira linhas em `quiz_perguntas` (5 opções, `correta` de 1 a 5).
...
- **Relay (recomendado p/ qualquer rede):** rode `encoder/relay` (servidor WS genérico):
  ```bash
...
- **Rede local direta:** abra o `quiz-biblico.html` salvo no PC (não em HTTPS) e informe o IP do ESP32.
